# Ackman Tracker (confluencia, no copia)

Datos reales en `config/ackman-tracker.json` (13F Pershing Square, refrescar cada quarter).
...
- Top reales: BN, AMZN, UBER, MSFT, QSR, META, HHH.
- **AMZN** = #2 de Ackman + su mayor add → confluencia con nuestro BUY.
- **GOOG** = cortado -94.9% (salió) → GOOGL degradado como backup.
- HOOD no está en su libro.

## Regla operativa

Antes de un BUY, consultar tracker:
- ¿Ackman lo tiene / añadió? → confluencia (+)
- ¿Ackman lo cortó / no lo tiene? → exigir tesis propia más fuerte
