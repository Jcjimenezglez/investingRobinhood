...
**Modo:** research-only — **NO trades**. Options OFF.

## Pre-flight

1. Leer `prompt/manifest.json` + secciones en `loadOrder`
2. Leer `config/signal-weights.json`, `config/fund-mandate.json`, `config/ackman-tracker.json`, `config/risk-policy.json` (`options.enabled=false`)
