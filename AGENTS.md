...
- Public site and Desk MCP copy must **not** name Kevin Xu. Describe the book as an **all-in strategy** (one listed stock, vibes + support + catalyst, never chase). Internal `prompt/`, `config/`, and `workflows/` keep the mandate name. `web/lib/sanitize.ts` redacts the name from published logs.
